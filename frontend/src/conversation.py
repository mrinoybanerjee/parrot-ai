...
# Define the maximum number of exchanges for different session lengths and learning modes
MAX_EXCHANGE_COUNTS = {
...
def generate_conversation(role_dict, language, scenario, proficiency_level, learning_mode, session_length):
    """
    Generates a conversation based on the provided parameters by sending a request to the backend server.
    ...
    Raises:
        requests.RequestException: If there is an error communicating with the backend server.
    """
    try:
...
def setup_conversation(conversation_container, translate_col, original_col, audio_col, learning_mode,
    ...
    """
    Sets up the conversation UI and handles the logic for generating and displaying the conversation.

    Args:
        conversation_container (streamlit.container): Streamlit container to display the conversation.
        ...
        with conversation_container:
            ...
            with st.spinner('Generating conversation...'):
                for _ in range(MAX_EXCHANGE_COUNTS[session_length][learning_mode]):
                    result = generate_conversation(role_dict, language, scenario, proficiency_level, learning_mode, session_length)
                    if result:
                        output1, output2, translate1, translate2 = result["response1"], result["response2"], result["translate1"], result["translate2"]
